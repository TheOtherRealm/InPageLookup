...
(function () {
	...
	var getSelectedPedia = function (c) {
		selObj = window.getSelection();
		nOfLookups++;
		$('#wikiWrapper'+nOfLookups).append('<div class="wikiAddonDivRap" id="' + nOfLookups + '" style="position: fixed;  top:' + (nOfLookups * 10) + 'px;left:' + (nOfLookups * 10) + 'px"">' +
	...
	$('body').prepend('<div id="wikiWrapper'+nOfLookups+'"></div>');
	...
	var closeWiki = function (c) {
		if ($('#wikiWrapper'+nOfLookups+'>*').length <= 0) {
			parent.postMessage('closeWiki','*');
		} else {
			$('#wikiWrapper'+nOfLookups+'>*').remove();
			nOfLookups--;
		}
	};
})(jQuery);